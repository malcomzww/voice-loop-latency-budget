...
def write_committed(data: dict) -> None:
    ...
    add(f"  the first chunk is playable within **{fc_bound:.0%}** of total")
    add("  synthesis time at p50.")
    add(f"- Synthesis runs at least **{band(audio / tot, (5, 10)):.0f}x faster than")
    add("  real time**, so playback of chunk *n* covers synthesis of chunk *n+1*")
    add("  and the stream never underruns.")
